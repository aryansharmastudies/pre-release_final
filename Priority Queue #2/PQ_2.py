...
def FindFreeTill(Tills, NoOfTills, BuyerQ):
  ThisBuyerItems = BuyerQ[0].ItemsInBasket
  FreeTills = list()
  
  for till in range(1, NoOfTills + 1): 
    if Tills[till][TIME_SERVING] == 0: 
      FreeTills.append(till)
  
  print(FreeTills)
  if len(FreeTills) == 1:
    return FreeTills[0]
  elif len(FreeTills) > 1 and 1 in FreeTills:
    if ThisBuyerItems < 8: 
      return 1
    else: 
      return FreeTills[1]
...
def Serving(Tills, NoOfTills, BuyerQ, QLength, Stats):
  TillFree = FindFreeTill(Tills, NoOfTills, BuyerQ)
