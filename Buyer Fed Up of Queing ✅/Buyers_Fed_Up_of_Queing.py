  ...
  for Count in range(QLength):
    ...
    if BuyerQ[Count].WaitingTime == 3:
      ...
      for i in range(0, QLength - 1):
        ...
        BuyerQ[i].WaitingTime = BuyerQ[i + 1].WaitingTime
        BuyerQ[i].ItemsInBasket = BuyerQ[i + 1].ItemsInBasket
      BuyerQ[QLength - 1].BuyerID = BLANK
      BuyerQ[QLength - 1].WaitingTime = 0
      BuyerQ[QLength - 1].ItemsInBasket = 0
      QLength -= 1
...
def Serving(Tills, NoOfTills, BuyerQ, QLength, Stats):
  ...
  while TillFree != -1 and QLength > 0:
    ...
    TillFree = FindFreeTill(Tills, NoOfTills)
  BuyerQ, QLength = IncrementTimeWaiting(BuyerQ, QLength)
